...
async function runVisualRegionFill(
  ...
  try {
    ...
    status.update(`AI 正在识别 ${controls.length} 个字段并按插件资料覆盖...`);
...
function collectVisualControlBindings(root: ParentNode): VisualControlBinding[] {
  ...
  return elements.flatMap(element => {
    if (!isWritableControl(element)
      || !isPrimaryCustomSelectControl(element)
      || !isVisibleFormControl(element)) {
      return [];
    }

    const visualElement = element.offsetParent !== null
      ? element
      : getCustomSelectRoot(element) || element;
    ...
    const container = element.closest<HTMLElement>(
      `${FIELD_CONTAINER_SELECTOR}, label`,
    );
    ...
    const label = (identifiers.labelText || findLabelText(element)).trim();
    ...
    return [{
      ...
      label,
      name,
      semanticType: match.fieldType,
      rowIndex: FieldMatcher.extractRowIndex(element),
      placeholder: element.getAttribute('placeholder') || '',
      options: collectControlOptions(element, label, name, match.fieldType as FieldType),
      rect: {
        left: rect.left,
        top: rect.top,
        width: rect.width,
...
function collectControlOptions(
  ...
  if (fieldType === FieldType.EDUCATION_COUNTRY || fieldType === FieldType.NATIONALITY) {
    return ['中国', '中国香港', '中国澳门', '中国台湾', '其他'];
  }
  if (fieldType === FieldType.EDUCATION_TYPE || label === '学历类型' || name === 'education_type') {
    return ['海外及港澳台', '统招全日制', '统招非全日制', '自考', '其他'];
  }
  if (fieldType === FieldType.DEGREE || label === '学历' || name === 'degree') {
