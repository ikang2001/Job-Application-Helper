...
test('表格顶部提供常驻公司搜索且不重复放置岗位和状态搜索框', async () => {
  ...
  assert.ok(findInputByAriaLabel(renderer.root, '搜索已投递公司'));
...
test('同一天的记录默认及投递日期降序都把最晚投递放在最上面', async () => {
  const sameDayRecords: ApplicationRecord[] = [
    {
      ...records[0],
      id: 'same-day-early',
      companyName: '上海巨人',
...
test('待确认投递支持勾选多条并一次批量忽略', async () => {
  ...
  const context = {
    ...
    metadata: { companyName: '示例公司', jobTitle: '后端工程师' },
  } as SubmissionContext;
  const candidates = ['pending-1', 'pending-2'].map((id, index) => ({
    pending: {
      id,
    ...
    context: {
      ...context,
      id: `context-${index + 1}`,
      metadata: { companyName: `候选公司${index + 1}`, jobTitle: '开发工程师' },
    },
  }));
  candidates.forEach(item => { item.pending.contextId = item.context.id; });

  await withMockedSendMessage(async message => {
    sentMessages.push(message);
    if (message.type === 'GET_APPLICATION_RECORDS') return { success: true, data: records };
    if (message.type === 'GET_PENDING_SUBMISSIONS') return { success: true, data: candidates };
    if (message.type === 'IGNORE_PENDING_SUBMISSIONS') return { success: true, data: { ignored: 2 } };
    return { success: true, data: null };
  }, async () => {
    let renderer!: TestRenderer.ReactTestRenderer;
    await act(async () => {
      renderer = TestRenderer.create(<ApplicationRecordsSection />);
      await new Promise(resolve => setTimeout(resolve, 0));
    });

    await act(async () => {
      findInputByAriaLabel(renderer.root, '全选投递候选').props.onChange();
    });
    await act(async () => {
      await findButton(renderer.root, '批量忽略（2）').props.onClick();
    });

    const batchMessage = sentMessages.find(message => message.type === 'IGNORE_PENDING_SUBMISSIONS');
    assert.deepEqual(
      (batchMessage as Extract<Message, { type: 'IGNORE_PENDING_SUBMISSIONS' }>).payload.pendingIds,
      ['pending-1', 'pending-2'],
    );
  });
});
