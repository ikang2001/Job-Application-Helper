...
function defaultSort(records: ApplicationRecord[]): ApplicationRecord[] {
  return [...records]
    ...
    .sort((left, right) => compareApplicationRecordSubmissionTime(left, right));
}
...
function applySort(records: ApplicationRecord[], sortState: SortState): ApplicationRecord[] {
  ...
  const sorted = [...records].sort((left, right) => {
    for (const rule of sortState) {
      if (rule.key === 'appliedAt') {
        const comparison = compareApplicationRecordSubmissionTime(
          left,
          right,
          rule.direction === 'asc',
        );
...
export function ApplicationRecordsSection({
  ...
  const loadRecords = async () => {
    ...
    const pendingResponse = await MessageService.sendMessage<Array<{
      pending: PendingSubmission;
      context: SubmissionContext;
    ...
    if (pendingResponse.success) {
      ...
      setSelectedPendingIds(current => {
        const available = new Set(nextPending.map(item => item.pending.id));
  ...
  useEffect(() => {
    if (initialRecords.length > 0 || typeof chrome === 'undefined' || !chrome.storage?.onChanged) return;
    const handleRecordsChange = (
      changes: Record<string, chrome.storage.StorageChange>,
      areaName: string,
    ) => {
      if (areaName !== 'local' || !changes.applicationRecords) return;
      setRecords(defaultSort(normalizeApplicationRecords(changes.applicationRecords.newValue)));
    };
    chrome.storage.onChanged.addListener(handleRecordsChange);
    return () => chrome.storage.onChanged.removeListener(handleRecordsChange);
  }, [initialRecords.length]);

  const filteredRecords = useMemo(() => {
    const companyKeyword = normalizeKeyword(companySearch);
    const nextRecords = records.filter(record => (
      matchesKeyword(record.companyName, companyKeyword)
  ...
  }, [columnFilters, companySearch, records, sortState]);
  ...
  const decidePendingSubmission = async (pendingId: string, action: 'confirm' | 'ignore') => {
    setBusyAction('save');
    const response = action === 'confirm'
      ? await MessageService.sendMessage({ type: 'CONFIRM_PENDING_SUBMISSION', payload: { pendingId } })
      : await MessageService.sendMessage({ type: 'IGNORE_PENDING_SUBMISSION', payload: { pendingId } });
    if (!response.success) setErrorText(response.error || '处理投递候选失败');
    else {
      setNotice({ type: 'success', text: action === 'confirm' ? '投递记录已保存' : '投递候选已忽略' });
      await loadRecords();
    }
    setBusyAction(null);
  };
  ...
  const toggleAllPending = () => {
    setSelectedPendingIds(current => (
      current.size === pendingSubmissions.length
        ? new Set()
        : new Set(pendingSubmissions.map(item => item.pending.id))
    ));
  };

  const ignoreSelectedPending = async () => {
    const pendingIds = [...selectedPendingIds];
    if (pendingIds.length === 0) return;
    if (
      typeof window !== 'undefined'
      && typeof window.confirm === 'function'
      && !window.confirm(`确定忽略选中的 ${pendingIds.length} 条投递候选吗？`)
    ) return;
    setBusyAction('save');
    setErrorText('');
    const response = await MessageService.sendMessage<{ ignored: number }>({
      type: 'IGNORE_PENDING_SUBMISSIONS',
      payload: { pendingIds },
    });
    if (!response.success) {
      setErrorText(response.error || '批量忽略投递候选失败');
    } else {
      const ignored = response.data?.ignored ?? pendingIds.length;
      setSelectedPendingIds(new Set());
      setNotice({ type: 'success', text: `已忽略 ${ignored} 条投递候选` });
      await loadRecords();
    }
    setBusyAction(null);
  };
  ...
  return (
    <section className="application-records-section">
      ...
      {pendingSubmissions.length > 0 && (
        <section className="pending-submission-list" aria-label="待确认投递">
          <div className="pending-submission-heading">
            <div><strong>待确认投递</strong><span>{pendingSubmissions.length} 条</span></div>
            <div className="pending-submission-batch-actions">
              <label>
                <input
                  type="checkbox"
                  aria-label="全选投递候选"
                  checked={selectedPendingIds.size === pendingSubmissions.length}
              ...
              <button
                type="button"
                className="btn btn-secondary pending-submission-batch-ignore"
                onClick={() => void ignoreSelectedPending()}
                disabled={busyAction !== null || selectedPendingIds.size === 0}
              >
                批量忽略{selectedPendingIds.size > 0 ? `（${selectedPendingIds.size}）` : ''}
              </button>
            </div>
          </div>
          {pendingSubmissions.map(({ pending, context }) => (
            <article key={pending.id} className={selectedPendingIds.has(pending.id) ? 'is-selected' : ''}>
              <label className="pending-submission-selector">
                <input
                  type="checkbox"
                  aria-label={`选择投递候选：${context.metadata.companyName || '未识别公司'}`}
                  checked={selectedPendingIds.has(pending.id)}
                  onChange={() => togglePendingSelection(pending.id)}
                  disabled={busyAction !== null}
                />
              </label>
              <div className="pending-submission-content">
                <strong>{context.metadata.companyName || '未识别公司'} · {context.metadata.jobTitle || '未识别岗位'}</strong>
                <p>{context.sourceUrl}</p>
                <small>置信度 {Math.round(pending.score * 100)}% · {context.resumeProfileName || '未记录简历'} · {context.applicationEmail || '未记录邮箱'}</small>
              </div>
              <div>
                <button type="button" className="btn btn-primary" onClick={() => void decidePendingSubmission(pending.id, 'confirm')} disabled={busyAction !== null}>保存</button>
                <button type="button" className="btn btn-secondary" onClick={() => void decidePendingSubmission(pending.id, 'ignore')} disabled={busyAction !== null}>忽略</button>
              </div>
            </article>
          ))}
        </section>
      ...
      <div className="application-records-table-shell" aria-live="polite">
        <div className="application-records-table-toolbar">
          <label className="application-records-company-search">
            <span>公司</span>
            <input
              type="search"
              value={companySearch}
          ...
          <div>
            <span className="application-records-result-count">{filteredRecords.length} 条</span>
            <button
              type="button"
              className="btn btn-secondary"
                  ...
                  <tr key={record.id} className="application-records-row" data-row-type="data">
                    ...
                    <td className="application-records-cell-actions">
                      <div className="application-records-row-actions">
                        <button
                          type="button"
                          className="application-record-detail-button"
