...
const LOCAL_STATUS_LABELS: Record<LocalDesktopSyncState['status'], string> = {
  checking: '正在检测',
  syncing: '同步中',
  synced: '已连接桌面版',
  unavailable: '本地组件不可用',
...
function Summary({ summary }: { summary: BackupSummary }) {
  return (
    <dl className="backup-summary">
      ...
      <div><dt>AI 配置 / API Key</dt><dd>{summary.hasLLMConfig ? (summary.hasApiKey ? '旧备份含 Key（仅导入迁移）' : '配置包含，Key 不导出') : '不包含'}</dd></div>
      <div><dt>WebDAV 同步设置</dt><dd>{summary.hasWebDAVConfig ? '旧备份含凭据（仅导入迁移）' : '配置不含密码'}</dd></div>
    </dl>
  );
}
...
export function DataSyncSettings({ onDataChanged }: Props) {
  ...
  const [localStatus, setLocalStatus] = useState<LocalDesktopSyncState>({ status: 'checking' });
  const [busy, setBusy] = useState<string | null>(null);
  const [notice, setNotice] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  const refreshStatus = async () => {
    const [webDavResponse, localResponse] = await Promise.all([
      MessageService.sendMessage<SyncMetadata>({ type: 'GET_SYNC_STATUS' }),
      MessageService.sendMessage<LocalDesktopSyncState>({ type: 'GET_LOCAL_DESKTOP_SYNC_STATUS' }),
    ]);
    if (webDavResponse.success && webDavResponse.data) setMetadata(webDavResponse.data);
    if (localResponse.success && localResponse.data) setLocalStatus(localResponse.data);
  };
  ...
  const syncLocalNow = () => run('local-sync', async () => {
    const response = await MessageService.sendMessage<LocalDesktopSyncState>({
      type: 'SYNC_LOCAL_DESKTOP_NOW',
    });
    if (!response.success || !response.data) {
      setNotice({ type: 'error', text: response.error || '本机桌面同步失败' });
      return;
    }
    setLocalStatus(response.data);
    if (response.data.status === 'synced') onDataChanged();
    setNotice({
      type: response.data.status === 'synced' ? 'success' : 'error',
      text: response.data.status === 'synced'
        ? `已与桌面版同步 ${response.data.recordCount ?? 0} 条投递记录`
        : response.data.lastError || '本机桌面同步失败',
    });
  });
    ...
    <div className="data-sync-settings">
      <section className="data-section">
        <div className="data-section-heading">
          <div>
            <h2 className="settings-section-title">本机桌面同步</h2>
            <p className="settings-description">Edge 扩展与“秋招投递管理器”自动双向同步投递记录，无需导入文件或配置 WebDAV。</p>
          </div>
          <span className={`sync-status sync-status-${localStatus.status}`}>
            {LOCAL_STATUS_LABELS[localStatus.status]}
          </span>
        </div>
        <div className="sync-detail" aria-live="polite">
          <span>
            最近同步：{localStatus.lastSyncedAt ? new Date(localStatus.lastSyncedAt).toLocaleString() : '正在等待首次同步'}
            {typeof localStatus.recordCount === 'number' ? ` · ${localStatus.recordCount} 条记录` : ''}
          </span>
          {localStatus.filePath && <p>共享文件：{localStatus.filePath}</p>}
          {localStatus.lastError && <p>{localStatus.lastError}</p>}
        </div>
        <div className="sync-actions">
          <button className="btn btn-primary" onClick={syncLocalNow} disabled={busy !== null}>
            {busy === 'local-sync' ? '同步中…' : '立即与桌面版同步'}
          </button>
        </div>
        <p className="settings-hint">仅同步投递记录；API Key、邮箱授权码、WebDAV 密码、简历文件和其他设置不会写入共享文件。</p>
      </section>
      ...
      <section className="data-section">
        <div className="data-section-heading">
          <div>
            ...
            <p className="settings-description">导入或导出全部简历资料、简历原文件、AI 配置、通用设置和投递记录。</p>
          </div>
        ...
        <div className="sensitive-warning" role="note">
          新版本备份为明文 JSON，包含个人信息、简历原文件和投递记录，但不导出 AI API Key 或 WebDAV 密码。导入旧版备份时若发现凭据，会在确认后迁移到本机配置；请仍妥善保管备份文件。
        </div>
