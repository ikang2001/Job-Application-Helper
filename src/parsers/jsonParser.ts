...
export function parseResumeJSON(jsonText: string): ParsedResumeData {
  ...
  const personal: ParsedResumeData['personal'] = {
    ...
    phoneCountryCode: basic.phoneCountryCode || basic.phoneCode || '',
    phone: basic.phone || '',
    email: basic.email || '',
    idType: basic.idType || '',
    idCard: basic.idCard || basic.idNumber || '',
    nationality: basic.nationality || basic.areaCitizenshipName || basic.areaCitizenship || '',
  ...
  const education = (data.education ?? [])
    .map((e: any) => ({
      ...
      school: e.school || '',
      countryRegion: e.countryRegion || e.schoolCountry || e.country || e.countryName || '',
  ...
  const languages = (data.languages ?? data.languageList ?? [])
    .map((language: any) => ({
      id: language.id || language.resumeLanguageId || crypto.randomUUID(),
      language: language.language || language.languageName || language.otherLang || '',
      certificate: language.certificate || language.languageEvaluation || language.englishLevel || '',
      level: language.level || language.languageLevel || language.languageScore || language.englishScore || '',
    }))
    .filter((language: any) => language.language || language.certificate || language.level);
  ...
  const experience = (data.experience ?? [])
    .map((e: any) => {
      ...
      return {
        ...
        startDate,
        endDate,
        description: normalizeDescriptionText(stripHtml(e.details || e.description || '')),
      };
    });
  ...
  const projects = (data.projects ?? [])
    .map((p: any) => {
      ...
      const description = normalizeDescriptionText(stripHtml(p.description || ''));
      return {
  ...
  const awards = (data.awards ?? [])
    .map((award: any) => ({
      ...
      role: award.role || '',
      date: award.date || '',
      description: normalizeDescriptionText(stripHtml(award.description || '')),
    }))
