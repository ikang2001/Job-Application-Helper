import { FieldType, type UserProfile, type VisualRegionFillPayload } from '../../shared/types';
...
输出格式要求：
{
  ...
  "education": [
    {
      "school": "学校名称",
      "countryRegion": "学校所在国家/地区",
      "schoolLocation": "学校所在地（省/市/区县行政区路径）",
      "college": "学院/院系",
  ...
  "languages": [
    {
      "language": "外语语种（如英语）",
      "certificate": "证书/考试类型（如大学英语六级、雅思）",
      "level": "等级或成绩（如通过、熟练、7.0）"
    }
  ],
  "experience": [
    {
...
- 公司名不一定含"公司/集团"（如"示例科技""云帆电商""招聘平台"），按机构名识别
...
export function buildFieldMatchingPrompt(
  ...
  const fieldTypes = [
    'name', 'gender', 'birthDate', 'phoneCountryCode', 'phone', 'email', 'wechat', 'idType', 'idCard',
    'nationality', 'politicalStatus', 'ethnicity', 'hometown', 'currentAddress',
    'selfEvaluation', 'school', 'educationCountry', 'schoolLocation', 'college', 'educationType', 'major', 'degree', 'gpa', 'educationStartDate', 'graduationDate',
    'language', 'languageCertificate', 'languageLevel',
    ...
    'awardName', 'awardRole', 'awardDate', 'awardDescription',
  ];
...
export function buildSectionFillPrompt(
  ...
  const profileContext = buildLLMProfileContext(profile, JSON.stringify({
    section: payload.section,
    fields: payload.fields,
  }));
  const semanticTypes = Object.values(FieldType)
    .filter(type => type !== FieldType.UNKNOWN && type !== FieldType.RESUME_FILE)
    .join(', ');
  const system = `你是网申表单补填助手。根据候选人已有资料，为当前模块仍为空的字段选择对应值。

严格规则：
- 只能使用候选人资料中明确存在的信息，不得编造学校、公司、日期、证件、成绩或经历
- semanticType 不是 unknown 时，以它作为字段含义，输出时必须原样保留
- semanticType 是 unknown 时，根据 name、label、type、options、context 判断字段类型；只允许选择：${semanticTypes}
- 每个字段自己的 section 表示资料模块；rowIndex 从 0 开始，教育/实习/项目字段必须匹配相同模块、相同序号的记录
- phoneCountryCode 只能填写国家区号（如 +86），phone 只能填写不含国家区号的手机号
- idType 填证件类型，idCard 填证件号码；两者禁止互换
- language 填语种，languageCertificate 填考试/证书类型，languageLevel 填等级或成绩
- school、countryRegion、schoolLocation、college、major、degree、educationType 必须分别取同一条 education 记录的对应属性，禁止串位
- 姓/名拼音、英文名、第二专业/辅修、紧急联系人、家庭成员、推荐人、证明人、期望薪资等资料中没有独立值的字段必须留空，不能拿候选人本人的相似字段代填
- 如果字段提供 options，返回值必须与其中一个选项完全一致
- 无法确定时返回空字符串
- 日期使用 YYYY-MM
- 每个非空映射都必须同时返回字段 index、最终 semanticType 和 value
- 只返回严格 JSON，格式为 {"mappings":[{"index":0,"semanticType":"name","value":"张三"}]}，不要解释、不要 Markdown`;
...
请返回严格的 mappings JSON；无法填写的字段不要放入 mappings。`;
...
- semanticType 不是 unknown 时必须按它选择 matchedProfilePath；rowIndex 是重复经历在资料数组中的下标
- phoneCountryCode、phone、idType、idCard、language、languageCertificate、languageLevel 彼此独立，禁止串填
- school、countryRegion、schoolLocation、college、major、degree、educationType 必须取同一 rowIndex 的 education 属性
...
/**
 * 表单 AI 只接收当前任务需要的结构化资料。简历附件和解析全文永不进入
 * prompt；高敏感联系方式只在字段上下文明确请求时加入。
 */
export type LLMProfileContext = Omit<UserProfile, 'personal' | 'resume'> & {
  personal: Partial<UserProfile['personal']>;
};

export function buildLLMProfileContext(profile: UserProfile, requestedContext: string): LLMProfileContext {
  const normalizedContext = requestedContext.toLowerCase();
  const includeIdCard = /身份证|证件号|id\s*card|identity/.test(normalizedContext);
  const includePhone = /手机|电话|联系电话|phone|mobile|tel/.test(normalizedContext);
  const includeEmail = /邮箱|邮件地址|e-?mail/.test(normalizedContext);

  const personal: Partial<UserProfile['personal']> = { ...profile.personal };
  if (!personal.nationality) personal.nationality = resolvePersonalNationality(profile.personal);
  if (!includeIdCard) delete personal.idCard;
  if (!includePhone) delete personal.phone;
  if (!includeEmail) delete personal.email;

  const { resume: _resume, ...profileWithoutResume } = profile;
  const education = profileWithoutResume.education.map(item => ({
    ...item,
    countryRegion: resolveEducationCountryRegion(item),
  }));
  const experience = profileWithoutResume.experience.map(item => ({
    ...item,
    description: normalizeDescriptionText(item.description || ''),
  }));
  const projects = profileWithoutResume.projects.map(item => ({
    ...item,
    description: normalizeDescriptionText(item.description || ''),
  }));
  const awards = profileWithoutResume.awards.map(item => ({
    ...item,
    description: normalizeDescriptionText(item.description || ''),
  }));
  return { ...profileWithoutResume, education, experience, projects, awards, personal };
}
...
function buildProfileSummary(profile: UserProfile): string {
  ...
  if (profile.languages?.length) {
    parts.push(`外语：${profile.languages.map(item =>
      [item.language, item.certificate, item.level].filter(Boolean).join(' ')
    ).join('；')}`);
  }
