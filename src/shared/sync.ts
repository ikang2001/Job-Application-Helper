...
function normalizeBusinessData(data: BackupData): BackupData {
  ...
  return {
    ...
    // Secrets are credentials, not business state. Excluding them keeps an
    // old V2 baseline compatible with the V3 public backup representation.
    llmConfig: data.llmConfig ? { ...data.llmConfig, apiKey: '' } : null,
    applicationRecords: normalizeApplicationRecords(data.applicationRecords ?? []),
  };
}
