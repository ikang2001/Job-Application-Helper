...
export class NLPHelper {
  ...
  /**
   * 解析教育经历章节。
   * 典型行：「示例大学 理论经济学专业 · 硕士 2024/09 - 2027/06」
   * 学校、专业、学历、日期的先后顺序不固定，因此按特征各自识别，
   * 而不是依赖位置切分。
   */
  static parseEducationSection(lines: string[]): Partial<EducationInfo>[] {
    const degreeRegex = /(博士后|博士研究生|硕士研究生|博士|硕士|研究生|本科|学士|大专|专科|高中|中专|MBA|EMBA)/;
    ...
    for (const line of lines) {
      ...
      for (const part of parts) {
        ...
        if (!degree && degreeMatch && part.length <= 24 && !schoolRegex.test(part)) {
          ...
          const remainder = part
            ...
            .replace(/专业$/, '')
            .trim();
          if (remainder && !major && !schoolRegex.test(remainder)) major = remainder;
          continue;
        }
  ...
  /**
   * 解析实习/工作经历章节。
   * 条目头部行的特征是含起止日期（如「示例科技 测试岗位 2026/03 - 2026/06」），
   * 其后不含日期的行归为该条目的描述。这样不依赖公司名关键词，
   * 「示例科技」「云帆电商」「招聘平台」这类不含「公司/集团」的名称也能识别。
   */
  static parseExperienceSection(lines: string[]): Partial<ExperienceInfo>[] {
    ...
    const flushDescription = () => {
      if (entries.length > 0 && currentDescription.length > 0) {
        entries[entries.length - 1].description = normalizeDescriptionText(currentDescription.join('\n'));
      }
      currentDescription = [];
    };
  ...
  private static inferName(text: string): string {
    ...
    // 页眉常把姓名与联系方式排在一行，如「张三 example@qq.com 1380...」
    for (const line of lines.slice(0, 8)) {
